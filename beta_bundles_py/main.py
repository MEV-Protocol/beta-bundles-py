...
def handle_event(event):
    ...
    if event["address"] == os.environ.get("AUCTIONEER"):
        ...
        signature0 = w3.keccak(text='AuctionOpened(uint256,uint120)').hex()
        logger.debug(signature)
        logger.debug(signature0)
        if signature0 == event["topics"][0].hex():
            ...
            bidder.functions.openBid(os.environ.get("WEI_PER_GAS"), amount_of_gas, [hash]).transact()
        elif signature == event["topics"][0].hex():
            ...
            if bal > 0:
                ...
                bidder.functions.submitBundle(slot).transact()
